...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, x, y, inverted=False):
        ...
        self.passed = False  # New attribute to track if the bird has passed through the pipe
...
def start_game():
    bird = Bird()
    bird.rect.center = (WIDTH / 2, HEIGHT / 2)  # Set the bird's initial position to the center of the screen
...
# Game loop
running = True
playing = False  # Add a flag to track if the game is currently being played
restart_delay = 1000  # Delay in milliseconds before restarting the game
restart_time = pygame.time.get_ticks()  # Variable to keep track of the restart time
...
while running:
    ...
    # Check if the game is not currently being played and a restart delay has passed
    if not playing and pygame.time.get_ticks() - restart_time > restart_delay:
    ...
    if playing:
        ...
        if bird.check_boundaries():
            ...
            playing = False  # Reset the playing flag
            restart_time = pygame.time.get_ticks()  # Update the restart time
